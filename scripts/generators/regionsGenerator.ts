...
export class RegionsGenerator implements Generator {
  ...
  async generate(): Promise<void> {
    ...
    this.regions.forEach(async (region: Region) => {
      ...
      const regionSubdivisionsCodes = this.subdivisions
        .filter(
          (subdivision: Subdivision) =>
            region.countries.indexOf(subdivision.country) > -1,
        )
      ...
      const regionStreams = streams.filter((stream) =>
        stream.broadcastArea.intersects(regionCodes),
      );
      ...
      this.logger.info(
        JSON.stringify({ filepath, count: playlist.streams.count() }),
      );
    });
  }
}
