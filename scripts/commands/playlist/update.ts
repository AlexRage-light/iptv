...
async function main() {
  ...
  const output = processedIssues
    .map((issue_number) => `closes #${issue_number}`)
    .join(", ");
...
async function removeStreams(loader: IssueLoader) {
  ...
  issues.forEach((issue: Issue) => {
    ...
    const found: Stream = streams.first(
...
async function editStreams(loader: IssueLoader) {
  ...
  issues.forEach((issue: Issue) => {
    ...
    if (data.has("http_referrer"))
      stream.httpReferrer = data.get("http_referrer");
    if (data.has("channel_name")) stream.name = data.get("channel_name");
