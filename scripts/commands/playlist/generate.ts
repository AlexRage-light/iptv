...
async function main() {
  ...
  streams = streams.uniqBy(
    (stream: Stream) => (stream.channel || _.uniqueId()) + stream.timeshift,
  );
  logger.info(
    `found ${totalStreams} streams (including ${streams.count()} unique)`,
  );
  ...
  await new IndexNsfwGenerator({
    streams,
    logger: generatorsLogger,
  }).generate();
  logger.info("generating index.category.m3u...");
  await new IndexCategoryGenerator({
    streams,
    logger: generatorsLogger,
  }).generate();
  ...
  await new IndexLanguageGenerator({
    streams,
    logger: generatorsLogger,
  }).generate();
  logger.info("generating index.region.m3u...");
  await new IndexRegionGenerator({
    streams,
    regions,
    logger: generatorsLogger,
  }).generate();
  ...
  streams = streams
    ...
    .map((stream: Stream) => {
      ...
      if (channel) {
        ...
        const [_, countryCode] = file.name().match(/^([a-z]{2})(_|$)/) || [
          null,
          null,
        ];
        const defaultBroadcastArea = countryCode
