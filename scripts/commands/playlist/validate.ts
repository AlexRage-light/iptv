...
async function main() {
  ...
  const blocklist = new Collection(blocklistContent).map(
    (data) => new Blocked(data),
  );
  ...
  for (const filepath of groupedStreams.keys()) {
    ...
    const [, countryCode] = file.basename().match(/([a-z]{2})(|_.*)\.m3u/i) || [
      null,
      "",
    ];
    ...
    streams.forEach((stream: Stream) => {
      const channelNotInDatabase =
        stream.channel &&
        !channels.first((channel: Channel) => channel.id === stream.channel);
    ...
    if (log.notEmpty()) {
      ...
      log.forEach((logItem: LogItem) => {
        ...
        const status =
          logItem.type === "error" ? chalk.red(type) : chalk.yellow(type);

        logger.info(` ${chalk.gray(position)}${status}${logItem.message}`);
      });

      errors = errors.concat(
        log.filter((logItem: LogItem) => logItem.type === "error"),
      );
      warnings = warnings.concat(
        log.filter((logItem: LogItem) => logItem.type === "warning"),
      );
    }
  }
