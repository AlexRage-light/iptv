...
async function main() {
  ...
  addRequests.forEach((issue: Issue) => {
    ...
    else if (groupedChannels.missing(channelId))
      result.set("status", "invalid_id");
  ...
  editRequests.forEach((issue: Issue) => {
    ...
    else if (groupedStreams.missing(streamUrl))
      result.set("status", "invalid_link");
    else if (channelId && groupedChannels.missing(channelId))
      result.set("status", "invalid_id");
    else result.set("status", "pending");
